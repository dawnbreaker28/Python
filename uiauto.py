...
def test():
    ...
    edit = googleWindow.ComboBoxControl(searchDepth=5, Name='在 Google 上搜索，或者输入一个网址')
    print(edit.Name)
    print(1)
    try:
        # 获取EditControl支持的ValuePattern，并用Pattern设置控件文本为"Hello"
        edit.GetValuePattern().SetValue('https://zh.moegirl.org.cn/index.php?title=%E5%88%BB%E6%99%B4&variant=zh-hans'
                                      '&mobileaction=toggle_view_desktop')  # or edit.GetPattern(
        # edit.SetWindowText('https://zh.moegirl.org.cn/index.php?title=%E5%88%BB%E6%99%B4&variant=zh-hans'
